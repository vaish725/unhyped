...
/**
 * Interface for review test cases
 */
 ...
 */
const reviewTestCases: ReviewTestCase[] = [
  {
 ...
 */
async function runScraperTest(testCase: TestCase, page: any): Promise<TestResult> {
 ...
 */
async function runReviewTests(page: any): Promise<TestResult[]> {
  ...
  for (const testCase of reviewTestCases) {
    ...
    try {
      ...
      const passed: boolean = testCase.shouldPass; // Even 0 reviews is a pass for shouldPass tests
 ...
 */
async function runTikTokTests(page: any): Promise<TestResult[]> {
  const results: TestResult[] = [];
 ...
 */
async function generateTestReport(allResults: TestResult[]) {
