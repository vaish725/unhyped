 ...
 */
export async function scrapeAmazon(page: Page, url: string): Promise<{
  name: string;
  price: string;
  rating: string | null;
  reviews: string[];
  ...
  try {
    ...
    const name = await page.$eval('#productTitle', (el: Element) => 
    ...
    try {
      rating = await page.$eval('[data-hook="average-star-rating"] .a-icon-alt', (el: Element) => 
    ...
    let reviews: string[] = [];
    try {
      reviews = await page.$$eval('[data-hook="review-body"] span', (els: Element[]) =>
