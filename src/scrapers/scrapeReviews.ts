...
export interface ReviewData {
  platform: 'reddit' | 'sephora' | 'generic';
  author: string;
  rating: number | null;
  title: string;
  content: string;
...
/**
 * Interface for aggregated review summary
 */
export interface ReviewSummary {
  ...
  top_helpful_reviews: ReviewData[];
}
 ...
 */
export async function scrapeSephoraReviews(page: Page, productUrl: string): Promise<ReviewData[]> {
  ...
  try {
    ...
    const processedReviews: ReviewData[] = reviews.map(review => ({
 ...
 * @returns Array of review data
 */
export async function scrapeRedditReviews(page: Page, productName: string): Promise<ReviewData[]> {
 ...
 */
function analyzeSentiment(content: string, rating?: number | null): 'positive' | 'negative' | 'neutral' {
 ...
 */
export function generateReviewSummary(reviews: ReviewData[]): ReviewSummary {
  if (reviews.length === 0) {
    return {
      total_reviews: 0,
      average_rating: null,
...
function extractCommonPhrases(reviews: ReviewData[]): string[] {
