...
import { scrapeTikTok, TikTokVideoData } from './scrapers/scrapeTikTok.js';

puppeteer.use(StealthPlugin());

/**
 * Interface for standardized product data across all platforms
 */
export interface ProductData {
  ...
  platform: string;
  tiktok_data?: TikTokVideoData; // Optional TikTok metadata
}
...
function detectPlatform(url: string): { 
  type: 'product' | 'social' | 'unknown'; 
  platform: string; 
  requires_product_url?: boolean;
} {
 ...
 */
async function handleSocialMediaUrl(
  page: Page, 
  url: string, 
  platformInfo: { type: string; platform: string }
): Promise<{ social_data: TikTokVideoData; requires_product_url: boolean }> {
  
  console.log(chalk.cyan(`🎬 Analyzing ${platformInfo.platform} content for promotional indicators...`));
  
  let social_data: TikTokVideoData;
 ...
 */
async function scrapeGeneric(page: Page, url: string): Promise<ProductData> {
    ...
    for (const selector of genericSelectors.price) {
      try {
        price = await page.$eval(selector, (el: Element) => 
          el.textContent?.trim() || 'Price Not Available'
        );
