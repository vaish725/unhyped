...
export class RealityCheckEngine {
    ...
    async analyzeProduct(productData, socialData, reviewSummary) {
        ...
        const realityScore = this.calculateOverallScore(productAnalysis, socialAnalysis, ingredientAnalysis, reviewAnalysis, priceAnalysis);
        ...
        const { redFlags, greenFlags, recommendations } = this.generateInsights(productAnalysis, socialAnalysis, ingredientAnalysis, reviewAnalysis, priceAnalysis);
        const result = {
     ...
     */
    analyzeProduct_Internal(productData) {
        ...
        const brandRecognition = wellKnownBrands.some(brand => productNameLower.includes(brand)) ? 'well_known' : 'unknown';
        // Calculate availability score based on platform
        let availabilityScore = 50; // Default
        if (productData.platform === 'amazon')
            availabilityScore = 90;
        else if (productData.platform === 'sephora')
            availabilityScore = 85;
        else if (productData.platform === 'oliveyoung')
            availabilityScore = 70;
        else if (productData.platform === 'yesstyle')
     ...
     */
    analyzePrice(price, ingredientQualityScore) {
     ...
     */
    calculateOverallScore(product, social, ingredient, review, price) {
        ...
        const overallScore = (productScore * weights.product) +
     ...
     */
    determineVerdict(realityScore, socialAnalysis) {
        let verdict;
        let confidenceLevel;
        // Determine verdict
    ...
    calculateHashtagAuthenticity(hashtags) {
        ...
        if (spammyCount > organicCount)
            return 30;
        if (organicCount > spammyCount)
            return 85;
        return 60; // Mixed
    }
    ...
    calculateSponsoredProbability(socialData) {
        let probability = 0;
        if (socialData.paid_promotion_detected)
            probability += 40;
        if (socialData.affiliate_codes.length > 0)
            probability += 30;
        if (socialData.promotional_keywords.length > 0)
            probability += 20;
        // Hashtag analysis
        const sponsoredHashtags = socialData.hashtags.filter(tag => ['ad', 'sponsored', 'promo', 'affiliate', 'gifted'].includes(tag.toLowerCase()));
    ...
    findBeneficialIngredients(ingredients) {
        if (!this.ingredientsDatabase)
            return [];
        return ingredients.filter(ingredient => this.ingredientsDatabase.beneficial.some((beneficial) => ingredient.toLowerCase().includes(beneficial.toLowerCase())));
    }
    findHarmfulIngredients(ingredients) {
        if (!this.ingredientsDatabase)
            return [];
        return ingredients.filter(ingredient => this.ingredientsDatabase.potentially_harmful.some((harmful) => ingredient.toLowerCase().includes(harmful.toLowerCase())));
    }
    calculateIngredientQualityScore(beneficial, harmful, total) {
        if (total === 0)
            return 0;
    ...
    calculateIngredientAuthenticity(ingredients) {
        ...
        if (ingredients.parsed.length >= 5 && ingredients.parsed.length <= 50)
            score += 10;
        // Water/aqua typically first ingredient
        if (ingredients.parsed[0] && ['water', 'aqua'].includes(ingredients.parsed[0].toLowerCase()))
            score += 10;
        // Has preservatives (realistic for skincare)
        const hasPreservatives = ingredients.parsed.some(ing => ['phenoxyethanol', 'ethylhexylglycerin', 'benzyl alcohol'].includes(ing.toLowerCase()));
        if (hasPreservatives)
            score += 5;
        return Math.round(Math.max(0, Math.min(100, score)));
    }
    calculateReviewAuthenticity(sentimentBreakdown) {
        ...
        if (positiveRatio > 0.9 || negativeRatio > 0.8)
            return 30;
        // Good mix of sentiments indicates authenticity
        if (positiveRatio >= 0.4 && positiveRatio <= 0.8 && negativeRatio >= 0.1 && negativeRatio <= 0.4)
            return 85;
        return 60; // Average authenticity
    }
    assessReviewCredibility(reviewSummary, authenticityScore) {
        if (reviewSummary.total_reviews >= 20 && authenticityScore >= 70)
            return 'high';
        if (reviewSummary.total_reviews >= 5 && authenticityScore >= 50)
            return 'medium';
        return 'low';
    }
    assessPriceReasonableness(price) {
        const numericPrice = this.extractNumericPrice(price);
        if (numericPrice === null)
            return 50; // Unknown
        // Basic skincare price ranges (very rough estimates)
        if (numericPrice <= 15)
            return 95; // Very reasonable
        if (numericPrice <= 30)
            return 85; // Reasonable
        if (numericPrice <= 60)
            return 70; // Fair
        if (numericPrice <= 100)
            return 50; // Expensive but possibly justified
    ...
    assessValueForMoney(price, ingredientQuality) {
    ...
    calculatePriceIngredientRatio(price, qualityScore) {
    ...
    generateInsights(product, social, ingredient, review, price) {
        const redFlags = [];
        const greenFlags = [];
