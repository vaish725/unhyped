 ...
 */
export async function scrapeYesStyle(page, url) {
    ...
    try {
        ...
        for (const selector of nameSelectors) {
            try {
                name = await page.$eval(selector, (el) => el.textContent?.trim() || 'Unknown');
                if (name !== 'Unknown')
                    break;
            }
            catch {
        ...
        try {
            ...
            for (const selector of reviewSelectors) {
                try {
                    reviews = await page.$$eval(selector, (els) => els.slice(0, 5).map(el => el.textContent?.trim() || '').filter(Boolean));
                    if (reviews.length > 0)
                        break;
                }
                catch {
        ...
        try {
            ...
            if (!ingredientsRaw) {
                const detailSections = await page.$$eval('.product-details p, .pdp-details p, .js-product-info p', (els) => els.map(el => el.textContent?.trim() || ''));
